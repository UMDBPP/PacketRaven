...
class DatabaseCredentials(
    ConfigurationYAML, ConfigurationSection, PacketSourceConfiguration
):
    name = "database"
    fields = {
        "hostname": str,
    ...
    def from_connection_string(
        cls, connection_string: str, tunnel: SSHCredentials = None
    ):
        configuration = {}
        ...
        if connection_string is not None:
            ...
            if hostname.count("/") != 2:
                logging.warning(
                    f'unable to parse connection information from "{parsed}"'
                )
    ...
    def packet_source(self, callsigns: [str] = None) -> PacketDatabaseTable:
        ...
        if self["tunnel"] is not None:
            ssh_kwargs.update(
                {f"ssh_{key}": value for key, value in self["tunnel"].items()}
            )
