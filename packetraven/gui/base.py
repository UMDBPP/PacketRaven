...
class PacketRavenGUI:
    def __init__(self, configuration: RunConfiguration = None):
        ...
        log_file_label.grid(
            row=len(configuration_frame.grid_rows), column=0, sticky="w"
        )
        ...
        self.__toggles["log_to_file"].set(
            self.__configuration["log"]["filename"] is not None
        )
        ...
        self.__toggles["run_prediction"].set(
            self.__configuration["prediction"] is not None
        )
        ...
        if (
            ...
            self.prediction_filename = self.__configuration["prediction"]["output"][
                "filename"
            ]
        ...
        if not self.running:
            ...
            try:
                ...
                if (
                    ...
                    try:
                        ssh_tunnel_credentials = self.__configuration["packets"][
                            "database"
                        ]["tunnel"]
                        ...
                        database_credentials = self.__configuration["packets"][
                            "database"
                        ]
                        ...
                        self.database = database_credentials.packet_source(
                            callsigns=callsigns
                        )
        ...
        if self.running:
            try:
                    ...
                    try:
                        self.__predictions.update(
                            packet_track_predictions(
                                self.packet_tracks,
                                self.__configuration["prediction"],
                            )
                        )
                ...
                if (
                    ...
                    and self.__time_without_packets
                    >= self.__configuration["time"]["timeout"]
                ):
    ...
    def __add_callsign_window(self, callsign: str) -> teek.Window:
        ...
        self.__add_text_box(
            ...
            column=self.__elements[f"{callsign}.ground_speed"].grid_info()["column"]
            + 3,
        )
        ...
        self.__add_text_box(
            ...
            column=self.__elements[f"{callsign}.distance_downrange"].grid_info()[
                "column"
            ]
            + 3,
        )
        self.__add_text_box(
            ...
            column=self.__elements[f"{callsign}.distance_overground"].grid_info()[
                "column"
            ]
            + 3,
        )
