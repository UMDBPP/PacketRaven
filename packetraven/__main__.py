...
def packetraven_command(configuration_filename: str, gui: bool = False):
    ...
    if configuration_filename is not None:
        ...
        if (
            ...
            and configuration["time"]["end"] is None
        ):
            filter_message += (
                f' sent after {configuration["time"]["start"]:%Y-%m-%d %H:%M:%S}'
            )
        elif (
            configuration["time"]["start"] is None
            and configuration["time"]["end"] is not None
        ):
            filter_message += (
                f' sent before {configuration["time"]["end"]:%Y-%m-%d %H:%M:%S}'
            )
        elif (
            configuration["time"]["start"] is not None
            and configuration["time"]["end"] is not None
            ...
            for location in configuration["packets"]["text"]["locations"]:
                ...
                try:
                    if Path(location).suffix in [".txt", ".log"]:
                        connection = RawAPRSTextFile(
                            location, configuration["callsigns"]
                        )
                        ...
                        logging.info(
                            f"opened port {connection.location} for APRS parsing"
                        )
        ...
        if any(configuration["plots"].values()):
            ...
            for variable, enabled in configuration["plots"].items():
                if enabled and variable not in plots:
                    plots[variable] = LiveTrackPlot(
                        packet_tracks, variable, predictions
                    )
    ...
    if len(parsed_packets) > 0:
        ...
        for parsed_packet in parsed_packets:
            ...
            if callsign not in packet_tracks:
                packet_tracks[callsign] = APRSTrack(
                    packets=[parsed_packet], callsign=callsign
                )
...
def packet_track_predictions(
    ...
    for name, packet_track in packet_tracks.items():
        ...
        if (
            track_configuration["float"]["altitude"] is not None
            and not packet_track.falling
        ):
            packets_at_float_altitude = packet_track[
                numpy.abs(
                    track_configuration["float"]["altitude"] - packet_track.altitudes
                )
            ...
            if (
                ...
                track_configuration["float"]["start_time"] = packets_at_float_altitude[
                    0
                ].time
                track_configuration["profile"]["descent_only"] = False
            elif packet_track.ascent_rates[-1] >= 0:
                track_configuration["float"]["start_time"] = track_configuration[
                    "start"
                ]["time"] + timedelta(
                    seconds=(
    ...
    if all(
        ...
        try:
            prediction_query = CUSFBalloonPredictionQuery(
                ...
                sea_level_descent_rate=configuration["profile"][
                    "sea_level_descent_rate"
                ],
