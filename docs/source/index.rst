...
To run ``packetraven``, download the latest release from the `Releases page <https://github.com/UMDBPP/PacketRaven/releases>`_ 
and run the executable in the command line with your configuration file as the first argument:

.. code-block:: shell

   packetraven examples/example_1.yaml

The configuration file is in YAML format, as follows (all units are in SI):
